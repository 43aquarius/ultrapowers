...
用于展示原型、图表和选项的浏览器可视化头脑风暴伴侣。

## 使用时机

按问题决定，而非按会话。判断标准：**用户通过看是否比通过读理解得更好？**
...
- **概念性 A/B/C 选择**——在文字描述的方法之间挑选
- **权衡列表**——优缺点、比较表
- **技术决策**——API 设计、数据建模、架构方法选择
- **澄清性问题**——任何答案是文字而非视觉偏好的问题

关于 UI 主题的问题不自动等于视觉问题。"你想要哪种向导？"是概念性的——使用终端。"哪种向导布局感觉对？"是视觉性的——使用浏览器。

## 工作原理

服务器监视目录中的 HTML 文件，并将最新的一个提供给浏览器。你将 HTML 内容写入 `screen_dir`，用户在其浏览器中看到它，并可以点击选择选项。选择被记录到 `state_dir/events`，你在下一轮读取。

**内容片段与完整文档：** 如果你的 HTML 文件以 `<!DOCTYPE` 或 `<html` 开头，服务器按原样提供（仅注入辅助脚本）。否则，服务器自动将你的内容包装在框架模板中——添加头部、CSS 主题、选择指示器和所有交互基础设施。**默认写入内容片段。** 只有当你需要完全控制页面时才写完整文档。
...
**查找连接信息：** 服务器将其启动 JSON 写入 `$STATE_DIR/server-info`。如果你在后台启动了服务器但没有捕获 stdout，读取该文件以获取 URL 和端口。使用 `--project-dir` 时，检查 `<project>/.ultrapowers/brainstorm/` 中的会话目录。

**注意：** 将项目根目录作为 `--project-dir` 传递，使原型持久化在 `.ultrapowers/brainstorm/` 中并在服务器重启后存活。没有它，文件进入 `/tmp` 并被清理。如果还没有，提醒用户将 `.ultrapowers/` 添加到 `.gitignore`。
...
# 在 Bash 工具调用上使用 run_in_background: true，使服务器
# 在对话轮次之间存活。
scripts/start-server.sh --project-dir /path/to/project
```
...
scripts/start-server.sh --path/to/project
```
...
**其他环境：** 服务器必须在对话轮次之间在后台持续运行。如果你的环境会回收分离的进程，使用 `--foreground` 并使用你平台的后台执行机制启动命令。

如果 URL 从你的浏览器无法访问（在远程/容器化设置中常见），绑定一个非环回主机：
...
2. **告诉用户期望什么并结束你的轮次：**
   ...
   - 给出屏幕上内容的简要文本摘要（例如，"展示主页的 3 个布局选项"）
   - 要求他们在终端中回应："看看然后告诉我你的想法。如果想选择选项可以点击。"
...
4. **迭代或推进**——如果反馈改变当前屏幕，写一个新文件（例如 `layout-v2.html`）。只有当前步骤被验证后才移动到下一个问题。

5. **返回终端时卸载**——当下一步不需要浏览器时（例如，澄清问题、权衡讨论），推送一个等待屏幕以清除过期内容：
   ...
   这防止用户在对话已经继续时盯着已解决的选择。当下一个视觉问题出现时，照常推送新的内容文件。

6. 重复直到完成。

## 编写内容片段

只编写页面内部的内容。服务器自动将其包装在框架模板中（头部、主题 CSS、选择指示器和所有交互基础设施）。
...
**多选：** 在容器上添加 `data-multiselect` 以允许用户选择多个选项。每次点击切换该项。指示条显示计数。
...
当用户在浏览器中点击选项时，他们的交互被记录到 `$STATE_DIR/events`（每行一个 JSON 对象）。当你推送新屏幕时文件自动清除。
...
- **根据问题调整保真度**——布局用线框图，打磨问题用打磨级别
- **在每页上解释问题**——"哪个布局感觉更专业？"而非只是"选一个"
- **推进前先迭代**——如果反馈改变当前屏幕，写新版本
- **每屏最多 2-4 个选项**
- **在重要时使用真实内容**——对于摄影作品集，使用真实图片（Unsplash）。占位内容会掩盖设计问题
- **保持原型简单**——关注布局和结构，不追求像素级完美
...
如果会话使用了 `--project-dir`，原型文件会持久化在 `.ultrapowers/brainstorm/` 中供以后参考。只有 `/tmp` 会话在停止时被删除。
